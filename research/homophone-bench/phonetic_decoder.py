...
def decode(query_ipa: str, root: Node, top_n: int = 5, max_words: int = MAX_WORDS) -> list[dict]:
    ...
    if not q:
        return []
    nq = len(q)
    # state: (cost, qpos, node, path_len, words, wordlen, zipfsum, matched, baddel)
    start = (0.0, 0, root, 0, (), 0, 0.0, 0, 0, 0.0)
    beams: list[list] = [[] for _ in range(nq + 1)]
    beams[0] = [start]
    results: list[tuple[float, tuple, float]] = []

    for qpos in range(nq + 1):
        layer = heapq.nsmallest(BEAM, beams[qpos], key=lambda s: s[0])
        ...
        if qpos < nq:
            beams[qpos + 1] = heapq.nsmallest(BEAM * 2, beams[qpos + 1],
                                              key=lambda s: s[0])
